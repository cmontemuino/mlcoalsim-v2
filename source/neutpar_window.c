...
void calc_neutpar_window(struct var2 **inputp,struct dnapar *ntpar,long int  s0,long int  s1,double valuer,int npopa, char **dna_matrix, long int *posit)
{
    ...
    if(segsit == 0 || nsam < 2) {
        ...
        (ntpar)->Se1 = Se1;
        (ntpar)->Sn1 = Sn1;
        (ntpar)->pie1 = k_e1/comb;
        (ntpar)->pin1 = k_n1/comb;
        ...
        (ntpar)->m_sdev = s;
        if(s) {
            (ntpar)->m_skew = g1 * comb/((comb-2)*(comb-1)*s*s*s);
            (ntpar)->m_kurt = g2 * (comb-1) * comb /((comb-3)*(comb-2)*(comb-1)*s*s*s*s)
                              - (3. * (comb-1)*(comb-1))/((comb-3)*(comb-2));
        }
        else {
            (ntpar)->m_skew = -10000.;
            (ntpar)->m_kurt = -10000.;
        ...
        if((*inputp)->includeEHHrel) {
            ...
            for(k=ehh0;k<ehhs;k++) {
                ...
                for(j=1;j<segsit;j++) {
                    ...
                    if(l==-1) continue;
                    iHHA += ((EHSA[l]+EHSA[j])*(posit[s0+j]-posit[s0+l]))/2.;
                }
                for(j=1;j<segsit;j++) {
                    ...
                    if(l==-1) continue;
                    iHHD += ((EHSD[l]+EHSD[j])*(posit[s0+j]-posit[s0+l]))/2.;
                }
                if(iHHD != 0. && iHHA/iHHD > 0.) {
                    uiHS = log(iHHA/iHHD);
        ...
        if((*inputp)->includeEHHrel) {
            ...
            for(k=ehh0;k<ehhs;k++) {
                ...
                for(j=k;j<segsit;j++) {
                    if(flagM) {
                        ...
                        EHHS[k][j] = EHHS[j][k] = 1. - testHap(nsam,fhap_ij);
            ...
            for(k=ehh0;k<ehhs;k++) {
                for(j=1;j<segsit;j++) {
                    ...
                    if(l==-1) continue;
                    iES[k] += ((EHHS[k][l]+EHHS[k][j])*(posit[s0+j]-posit[s0+l]))/2.;
                }
        ...
        (ntpar)->thetaL = 0;
        for(a=1;a<nsam;a++) {
            (ntpar)->thetaL += (double)a * ntpar->freq[a];
        }
        (ntpar)->thetaL *= 1./(nsam-1);
    }
    ...
    if(npopa < (*inputp)->max_npop_sampled) {
        ...
        if((*inputp)->max_npop_sampled > 2 || ((*inputp)->max_npop_sampled == 2 && (*inputp)->pop_outgroup == -1)) {
            ...
            for(h=0;h<(*inputp)->npop_sampled;h++) {
                if((*inputp)->config[h]>1) (ntpar)->piwallcomp[h] = piw[h] / (((*inputp)->config[h] * ((*inputp)->config[h] - 1))/2.);
            }
            for(h=0;h<(*inputp)->npop_sampled;h++) {
                if((*inputp)->config[h] && (*inputp)->config[npopa] && npopa != h /**/ && h != (*inputp)->pop_outgroup/*Included to eliminate the outgroup in the analysis*/) {
                    ...
                    if((ntpar)->piaallcomp[h]) {
                        if((*inputp)->config[h] > 1 && (*inputp)->config[npopa] > 1)
                            (ntpar)->fstallcomp[h] = 1.0 - (((ntpar)->piwallcomp[h]+(ntpar)->piwallcomp[npopa])/2.)/(ntpar)->piaallcomp[h];
                        else {
                            if((*inputp)->config[h] == 1 && (*inputp)->config[npopa] > 1)
                                (ntpar)->fstallcomp[h] = 1.0 - (((ntpar)->piwallcomp[npopa])/2.)/(ntpar)->piaallcomp[h];
                            if((*inputp)->config[h] > 1 && (*inputp)->config[npopa] == 1)
                                (ntpar)->fstallcomp[h] = 1.0 - (((ntpar)->piwallcomp[h])/2.)/(ntpar)->piaallcomp[h];
                            if((*inputp)->config[h] == 1 && (*inputp)->config[npopa] == 1)
            ...
            for(h=0;h<(*inputp)->npop_sampled;h++) {
                if((*inputp)->config[h] && (*inputp)->config[npopa] && npopa != h /**/ && h != (*inputp)->pop_outgroup/*Included to eliminate the outgroup in the analysis*/) {
                    ...
                    if((ntpar)->hapaallcomp[h]) {
                        if((*inputp)->config[h] > 1 && (*inputp)->config[npopa] > 1)
                            (ntpar)->fsthapallcomp[h] = 1.0 - (((ntpar)->hapwallcomp[h]+(ntpar)->hapwallcomp[npopa])/2.)/(ntpar)->hapaallcomp[h];
                        else {
                            if((*inputp)->config[h] == 1 && (*inputp)->config[npopa] > 1)
                                (ntpar)->fsthapallcomp[h] = 1.0 - (((ntpar)->hapwallcomp[npopa])/2.)/(ntpar)->hapaallcomp[h];
                            if((*inputp)->config[h] > 1 && (*inputp)->config[npopa] == 1)
                                (ntpar)->fsthapallcomp[h] = 1.0 - (((ntpar)->hapwallcomp[h])/2.)/(ntpar)->hapaallcomp[h];
                            if((*inputp)->config[h] == 1 && (*inputp)->config[npopa] == 1)
            ...
            for(h=0;h<(*inputp)->npop_sampled;h++) {
                if((*inputp)->config[h]>1) {
                    comb = (int)((*inputp)->config[h] * ((*inputp)->config[h] - 1)/2.);
                    (ntpar)->piw += (double)piw[h]/comb;
                    comb2++;
                }
            }
            ...
            if(comb2) {
                ...
                npw=0;
                for(h=0;h<(*inputp)->npop_sampled;h++) npw +=(*inputp)->config[h]>1;
            }
            ...
            for(h=0;h<(*inputp)->npop_sampled;h++) {
                if((*inputp)->config[h] && (*inputp)->config[npopa] && npopa != h/**/ && h != (*inputp)->pop_outgroup/*Included to eliminate the outgroup in the analysis*/) {
                    ...
                    (ntpar)->pib += (double)pib[h]/comb;
                    comb2++;
                }
            }
            if(comb2) (ntpar)->pib = (ntpar)->pib/comb2;
